...
def load_featured_leads() -> pd.DataFrame:
    """
    Loads all records from leads_featured table.
    ...
    conn = sqlite3.connect(config.DB_NAME)
    try:
        df = pd.read_sql_query("SELECT * FROM leads_featured", conn)
        logger.info(f"Loaded {len(df):,} records from leads_featured.")
        return df
    except Exception as e:
        logger.error(f"Could not load leads_featured: {e}")
        return pd.DataFrame()
    finally:
        conn.close()
...
def reset_all_tables():
    ...
    tables = ["leads", "leads_clean", "leads_featured",
